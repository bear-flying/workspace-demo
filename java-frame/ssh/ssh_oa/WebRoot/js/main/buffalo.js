...
Buffalo.Default = {
	...
	showLoading : function(state) {
		this.loadingPane = $get("buffalo_loading");
...
/*			el.innerHTML="buffalo loading... ";*/
	...
	showError: function(transport) {
        this.errorPane = $get("buffalo_error");
        if (this.errorPane == null) {
            if (window.inerror) {
			}
			var el = document.createElement('DIV');
			el.setAttribute("id","buffalo_error");
//			el.style.cssText="font-size:11px;border:4px solid #FF3333;background-color:#fff;padding:4px;position:absolute;overflow:auto; right:10px; top:10px; width:" + paneWidth + "; height:" + paneHeight + "; z-index:1";      //update by chenwen at 20150508 防止WIN8时AJAX出错出现红色不好看，暂时注掉
	...
	showException: function(faultObj) {
		this.exceptionPane = $get("buffalo_exception");
...
Buffalo.View.prototype = {
	...
	switchPart: function(partId, viewName, addToHistory) {
		...
		if (this.addToHistory) {
			if ($get(Buffalo.View.HISTORY_IFRAME_ID)) {
				var iframesrc=$get(Buffalo.View.HISTORY_IFRAME_ID).src;
				...
				newUrl += "?" + viewName;
				$get(Buffalo.View.HISTORY_IFRAME_ID).src = newUrl;
			} else {
	...
	_showView: function(partId, viewPath, viewData) {
		...
		/* draw the html first */
		$get(partId).innerHTML = viewData.replace(regexp1, "");
